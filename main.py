...
with st.form("supplier_form"):
    ...
    products_services = st.text_area("Sản phẩm/Dịch vụ cung cấp", 
    ...
    with col3:
        payment_terms = st.text_input("Điều khoản thanh toán", 
                                    placeholder="VD: 30 ngày")
        delivery_time = st.text_input("Thời gian giao hàng", 
                                    placeholder="VD: 7-10 ngày")
        
    with col4:
        bank_account = st.text_input("Tài khoản ngân hàng", 
    ...
    with st.form("company_form"):
        ...
        with col1:
            company_name = st.text_input("Tên công ty", 
                                       value=st.session_state.company_info['name'])
            company_phone = st.text_input("Điện thoại", 
                                        value=st.session_state.company_info['phone'])
            company_email = st.text_input("Email", 
                                        value=st.session_state.company_info['email'])
        
        with col2:
            company_address = st.text_area("Địa chỉ", 
                                         value=st.session_state.company_info['address'])
            company_tax_code = st.text_input("Mã số thuế", 
                                            value=st.session_state.company_info['tax_code'])
...
with col2:
    today_updates = len([s for s in st.session_state.suppliers_data 
...
with col4:
    complete_profiles = len([s for s in st.session_state.suppliers_data 
...
if st.session_state.suppliers_data:
    ...
    with col3:
        sort_by = st.selectbox("Sắp xếp theo", 
                             ["Cập nhật gần nhất", "Tên A-Z", "Loại hình"])
    ...
    if search_term:
        filtered_data = [s for s in filtered_data 
                       if search_term.lower() in s.get('supplier_name', '').lower()]
    ...
    # Display data
    for i, supplier in enumerate(filtered_data):
